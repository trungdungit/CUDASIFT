...
public:
    ...
    void detectKeypoints(cv::cuda::GpuMat& keypoints, int scales)
    {
...
void SIFT_GPU::operator()(const cv::Mat& image, cv::cuda::GpuMat& keypoints)
{
    if (!image.empty())
    {
...
void SIFT_GPU::downloadKeypoints(cv::cuda::GpuMat& keypoints_GPU, std::vector<cv::KeyPoint>& keypoints_CPU)
{
